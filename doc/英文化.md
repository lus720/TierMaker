# TierMakerPlus 中英国际化设计方案

> **开发分支**: [en](file:///c:/Users/12192/TierMaker/src/utils/storage.ts#324-327) (基于 main 创建)
>
> 完成后合并回 main

## 1. 需求总结

| 需求 | 决策 |
|------|------|
| 域名策略 | 单域名 + 语言切换按钮 |
| 语言检测 | 浏览器语言自动检测，用户可手动切换 |
| 翻译方案 | vue-i18n（解耦、行业标准） |
| 数据隔离 | 中英共享 IndexedDB，作品数据不变 |
| 等级处理 | 用户自定义优先，否则按语言显示默认值 |

---

## 2. 现有功能影响分析

> [!IMPORTANT]
> **核心原则**：i18n 只影响 **UI 显示层**，不影响 **数据层** 和 **业务逻辑层**。

### 2.1 不受影响的功能

| 功能 | 原因 |
|------|------|
| **拖动排序** | [dragManager.ts](file:///c:/Users/12192/TierMaker/src/utils/dragManager.ts) 基于 DOM 操作和 `rowId` 识别容器，与语言无关 |
| **作品数据** | [AnimeItem](file:///c:/Users/12192/TierMaker/src/types.ts#12-28) 存储在 IndexedDB，不包含语言字段 |
| **图片裁剪** | [EditItemModal.vue](file:///c:/Users/12192/TierMaker/src/components/EditItemModal.vue) 的裁剪逻辑基于像素坐标，与语言无关 |
| **主题切换** | CSS 变量驱动，与语言无关 |
| **搜索功能** | API 调用（Bangumi/VNDB）与语言无关 |

### 2.2 需要处理的功能

#### 2.2.1 等级标签 (TierConfig)

**现状**：

```typescript
// types.ts
interface TierConfig {
  id: string    // 例如 "夯"
  label: string // 例如 "夯" (与 id 相同)
  color: string
  order: number
}
```

**问题**：[id](file:///c:/Users/12192/TierMaker/src/utils/storage.ts#29-38) 和 `label` 使用相同的中文字符串，导致：

1. 切换语言时 [id](file:///c:/Users/12192/TierMaker/src/utils/storage.ts#29-38) 也会变化（如果直接替换为英文等级）
2. 作品与等级的关联关系断裂（`Tier.id` 需要与 `TierConfig.id` 匹配）

**解决方案**：

```typescript
// 新增：检测是否为默认等级
function isDefaultTierConfig(configs: TierConfig[]): boolean {
  const labels = configs.map(c => c.label)
  const zhDefaults = ['夯', '顶级', '人上人', 'NPC', '拉完了']
  const enDefaults = ['S', 'A', 'B', 'C', 'D']
  return JSON.stringify(labels) === JSON.stringify(zhDefaults) ||
         JSON.stringify(labels) === JSON.stringify(enDefaults)
}

// 切换语言时：
// - 如果 isDefaultTierConfig() === true，替换为目标语言的默认等级
// - 否则保留用户自定义等级
```

**影响范围**：✅ 低风险（只在 `handleLanguageChange` 时处理）

---

#### 2.2.2 导出功能 (ExportModal)

**现状**：

```typescript
// storage.ts - exportAllData()
export interface ExportData {
  tiers: Tier[]
  tierConfigs: TierConfig[]  // 包含当前语言的等级配置
  title: string
  exportDate: string
  version: string
}
```

**问题**：

- 导出的 JSON 包含当前语言的等级标签
- 如果用户在中文界面导出，在英文界面导入，等级标签仍为中文

15:04:47 [vite] (client) hmr update /src/App.vue
15:04:47 [vite] (client) Pre-transform error: Element is missing end tag.
  Plugin: vite:vue
  File: C:/Users/12192/TierMaker/src/components/ConfigModal.vue:360:3
  358 |  
  359 |  <template>
  360 |    <div class="modal-overlay" @mousedown="handleMouseDown" @mouseup="handleMouseUp">
      |     ^
  361 |      <div class="modal-content" ref="modalContentRef">
  362 |        <div class="modal-header">

15:06:51 [vite] Internal server error: Element is missing end tag.
  Plugin: vite:vue
  File: C:/Users/12192/TierMaker/src/components/ConfigModal.vue:360:3
  358 |
  359 |  <template>
  360 |    <div class="modal-overlay" @mousedown="handleMouseDown" @mouseup="handleMouseUp">
      |     ^
  361 |      <div class="modal-content" ref="modalContentRef">
  362 |        <div class="modal-header">
      at createCompilerError (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:1378:17)
      at emitError (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:3014:5)
      at Object.onclosetag (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:2381:13)
      at Tokenizer.stateInClosingTagName (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:799:16)
      at Tokenizer.parse (C:\Users\12

**解决方案**：这是预期行为，因为：

1. 导出是"数据快照"，应保留用户当时的配置
2. 用户可在设置中手动修改等级标签

**影响范围**：✅ 无需修改逻辑，只需翻译 UI 文本

---

#### 2.2.3 导入功能 (ImportModal)

**现状**：

- 从 JSON 文件导入
- 从 VNDB 用户列表导入
- 从 Bangumi 季度列表导入

**问题**：导入时的提示文本需要翻译

15:04:47 [vite] (client) hmr update /src/App.vue
15:04:47 [vite] (client) Pre-transform error: Element is missing end tag.
  Plugin: vite:vue
  File: C:/Users/12192/TierMaker/src/components/ConfigModal.vue:360:3
  358 |  
  359 |  <template>
  360 |    <div class="modal-overlay" @mousedown="handleMouseDown" @mouseup="handleMouseUp">
      |     ^
  361 |      <div class="modal-content" ref="modalContentRef">
  362 |        <div class="modal-header">

15:06:51 [vite] Internal server error: Element is missing end tag.
  Plugin: vite:vue
  File: C:/Users/12192/TierMaker/src/components/ConfigModal.vue:360:3
  358 |
  359 |  <template>
  360 |    <div class="modal-overlay" @mousedown="handleMouseDown" @mouseup="handleMouseUp">
      |     ^
  361 |      <div class="modal-content" ref="modalContentRef">
  362 |        <div class="modal-header">
      at createCompilerError (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:1378:17)
      at emitError (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:3014:5)
      at Object.onclosetag (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:2381:13)
      at Tokenizer.stateInClosingTagName (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:799:16)
      at Tokenizer.parse (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:1157:16)
      at Object.baseParse (C:\Users\12192\TierMaker\node_modules\@vue\compiler-core\dist\compiler-core.cjs.js:3053:13)
      at Object.parse (C:\Users\12192\TierMaker\node_modules\@vue\compiler-dom\dist\compiler-dom.cjs.js:910:23)
      at Object.parse$1 [as parse] (C:\Users\12192\TierMaker\node_modules\@vue\compiler-sfc\dist\compiler-sfc.cjs.js:1824:24)
      at createDescriptor (file:///C:/Users/12192/TierMaker/node_modules/@vitejs/plugin-vue/dist/index.mjs:54:42)
      at handleHotUpdate (file:///C:/Users/12192/TierMaker/node_modules/@vitejs/plugin-vue/dist/index.mjs:1162:25)

**解决方案**：

```vue
<!-- 翻译前 -->
<p class="description">输入您的 VNDB 用户 ID 以导入您的视觉小说列表。</p>

<!-- 翻译后 -->
<p class="description">{{ t('import.vndbDesc') }}</p>
```

**影响范围**：✅ 纯 UI 翻译，不影响导入逻辑

---

#### 2.2.4 编辑作品 (EditItemModal)

**现状**：所有表单标签和按钮都是硬编码中文

**问题**：需要翻译约 15 个文本

**影响范围**：✅ 纯 UI 翻译，不影响编辑逻辑

---

#### 2.2.5 设置页面 (ConfigModal)

**现状**：包含等级配置编辑、主题切换、导出倍率等设置

**问题**：

1. 需要翻译所有标签和按钮
2. **重要**：需要添加语言选择下拉框

**新增功能**：

```vue
<div class="form-group">
  <label>{{ t('settings.language') }}</label>
  <select v-model="currentLocale" @change="handleLanguageChange">
    <option value="zh">中文</option>
    <option value="en">English</option>
  </select>
</div>
```

**影响范围**：⚠️ 中等风险（需要新增语言切换逻辑和等级联动）

---

### 2.3 风险矩阵

| 风险 | 等级 | 影响 | 缓解措施 |
|------|------|------|---------|
| 等级 ID 变化导致数据关联断裂 | 🔴 高 | 作品丢失 | 保持 `Tier.id` 不变，只修改 `TierConfig.label` |
| 翻译遗漏导致中英混显 | 🟡 中 | 用户体验差 | 创建完整的翻译文件清单 |
| 语言切换后拖动异常 | 🟢 低 | 无 | 拖动逻辑与语言无关 |
| 导入旧版数据兼容性 | 🟢 低 | 无 | [ExportData](file:///c:/Users/12192/TierMaker/src/utils/storage.ts#308-317) 结构不变 |

---

## 3. 架构设计

```
┌─────────────────────────────────────────────────────────┐
│  App.vue                                                 │
│  ├─ useI18n() ─────────────────────────────────────────┐│
│  │   └─ locale: ref<'zh' | 'en'>                       ││
│  │                                                      ││
│  ├─ 语言选择优先级：                                    ││
│  │   1. localStorage['user-language'] (用户手动选择)   ││
│  │   2. navigator.language (浏览器语言)                ││
│  │   3. 'zh' (默认)                                    ││
│  └──────────────────────────────────────────────────────┘│
│                                                          │
│  数据层 (与语言无关)                                     │
│  ├─ IndexedDB: tier-list-data                           │
│  ├─ localStorage: tier-config (等级配置)                │
│  └─ localStorage: user-language (语言偏好)              │
└─────────────────────────────────────────────────────────┘
```

---

## 4. 核心实现

### 4.1 vue-i18n 配置

#### [NEW] [i18n.ts](file:///c:/Users/12192/TierMaker/src/i18n/index.ts)

```typescript
import { createI18n } from 'vue-i18n'
import zh from './locales/zh.json'
import en from './locales/en.json'

function getDefaultLocale(): 'zh' | 'en' {
  // 1. 用户手动选择
  const saved = localStorage.getItem('user-language')
  if (saved === 'zh' || saved === 'en') return saved
  
  // 2. 浏览器语言
  const browserLang = navigator.language || ''
  if (browserLang.startsWith('en')) return 'en'
  
  // 3. 默认中文
  return 'zh'
}

export const i18n = createI18n({
  legacy: false,  // 使用 Composition API
  locale: getDefaultLocale(),
  fallbackLocale: 'zh',
  messages: { zh, en }
})
```

#### [NEW] [zh.json](file:///c:/Users/12192/TierMaker/src/i18n/locales/zh.json)

```json
{
  "header": {
    "title": "TierMakerPlus",
    "addItem": "添加作品",
    "settings": "设置",
    "export": "导出",
    "import": "导入"
  },
  "tierLabels": {
    "default": ["夯", "顶级", "人上人", "NPC", "拉完了"]
  },
  "settings": {
    "title": "设置",
    "theme": "主题",
    "language": "语言",
    "tierConfig": "等级配置",
    "reset": "重置设置",
    "clearData": "清空数据"
  },
  "search": {
    "placeholder": "搜索作品...",
    "source": "搜索源",
    "noResults": "无结果"
  }
}
```

#### [NEW] [en.json](file:///c:/Users/12192/TierMaker/src/i18n/locales/en.json)

```json
{
  "header": {
    "title": "TierMakerPlus",
    "addItem": "Add Item",
    "settings": "Settings",
    "export": "Export",
    "import": "Import"
  },
  "tierLabels": {
    "default": ["S", "A", "B", "C", "D"]
  },
  "settings": {
    "title": "Settings",
    "theme": "Theme",
    "language": "Language",
    "tierConfig": "Tier Config",
    "reset": "Reset Settings",
    "clearData": "Clear Data"
  },
  "search": {
    "placeholder": "Search...",
    "source": "Source",
    "noResults": "No results"
  }
}
```

### 4.2 等级标签处理逻辑

> [!CAUTION]
> **关键原则**：切换语言时只修改 `label`，绝不修改 [id](file:///c:/Users/12192/TierMaker/src/utils/storage.ts#29-38)，否则会导致 `Tier.id` 与 `TierConfig.id` 不匹配，作品数据丢失。

```typescript
// 判断当前等级是否为默认等级（中文或英文）
function isDefaultTierConfig(configs: TierConfig[]): boolean {
  const labels = configs.map(c => c.label)
  const zhDefaults = ['夯', '顶级', '人上人', 'NPC', '拉完了']
  const enDefaults = ['S', 'A', 'B', 'C', 'D']
  
  return JSON.stringify(labels) === JSON.stringify(zhDefaults) ||
         JSON.stringify(labels) === JSON.stringify(enDefaults)
}

// 切换语言时的等级处理（关键：只改 label，不改 id）
function handleLanguageChange(newLocale: 'zh' | 'en') {
  const configs = loadTierConfigs()
  
  if (isDefaultTierConfig(configs)) {
    // 用户使用默认等级，只更新 label 为目标语言
    const newLabels = newLocale === 'zh' 
      ? ['夯', '顶级', '人上人', 'NPC', '拉完了']
      : ['S', 'A', 'B', 'C', 'D']
    
    configs.forEach((config, i) => {
      config.label = newLabels[i]  // ✅ 只改 label
      // config.id 保持不变！    // ✅ id 不动
    })
    
    saveTierConfigs(configs)
  }
  // 用户自定义等级时，保持不变
  
  localStorage.setItem('user-language', newLocale)
  i18n.global.locale.value = newLocale
}
```

**数据关系示意**：

```
切换语言前 (中文):
├─ TierConfig: { id: "夯", label: "夯" }
└─ Tier: { id: "夯" }  ← 通过 id 匹配

切换语言后 (英文):
├─ TierConfig: { id: "夯", label: "S" }  ← 只改了 label
└─ Tier: { id: "夯" }  ← 仍然匹配！作品不丢失
```

### 4.3 需要翻译的组件清单

| 组件 | 需翻译内容 |
|------|-----------|
| [App.vue](file:///c:/Users/12192/TierMaker/src/App.vue) | 标题、按钮文本 |
| [ConfigModal.vue](file:///c:/Users/12192/TierMaker/src/components/ConfigModal.vue) | 设置页所有标签、按钮、提示文本 |
| [SearchModal.vue](file:///c:/Users/12192/TierMaker/src/components/SearchModal.vue) | 搜索框占位符、搜索源选项、无结果提示 |
| [EditItemModal.vue](file:///c:/Users/12192/TierMaker/src/components/EditItemModal.vue) | 编辑框标题、保存/取消按钮 |
| [ExportModal.vue](file:///c:/Users/12192/TierMaker/src/components/ExportModal.vue) | 导出选项、按钮文本 |
| [ImportModal.vue](file:///c:/Users/12192/TierMaker/src/components/ImportModal.vue) | 导入说明、按钮文本 |
| [TierRow.vue](file:///c:/Users/12192/TierMaker/src/components/TierRow.vue) | 删除按钮 title |

---

## 5. 实现步骤

### Phase 1: 基础设施

- [ ] 安装 vue-i18n 依赖
- [ ] 创建 `src/i18n/` 目录结构
- [ ] 创建 zh.json 和 en.json 翻译文件
- [ ] 在 main.ts 中注册 i18n 插件

### Phase 2: 语言切换 UI

- [ ] 在 ConfigModal 添加语言选择下拉框
- [ ] 实现 `handleLanguageChange()` 函数
- [ ] 处理等级标签的语言联动逻辑

### Phase 3: 组件翻译

- [ ] 逐个组件替换硬编码文本为 [t('key')](file:///c:/Users/12192/TierMaker/src/utils/configManager.ts#193-206)
- [ ] 提取所有用户可见文本到翻译文件

### Phase 4: 验证

- [ ] 切换语言后 UI 正确显示
- [ ] 作品数据不变
- [ ] 自定义等级不被覆盖
- [ ] 首次访问时正确检测浏览器语言

---

## 6. 关键注意事项

> [!WARNING]
> **等级 ID 与标签分离**
>
> 当前 `tierConfigs` 的 [id](file:///c:/Users/12192/TierMaker/src/utils/storage.ts#29-38) 和 `label` 是相同的（如 `id: '夯', label: '夯'`）。
> 国际化后需要：
>
> - [id](file:///c:/Users/12192/TierMaker/src/utils/storage.ts#29-38) 保持为语言无关的标识符（如 `tier-0`, `tier-1`）
> - `label` 存储用户可见的文本

> [!IMPORTANT]
> **config.yaml 的 tiers 字段**
>
> 需要拆分为两份语言配置，或改为从 i18n JSON 读取默认值。

---

## 7. 依赖变更

```bash
npm install vue-i18n@9
```

---

## 8. 文件变更清单

| 操作 | 文件路径 |
|------|---------|
| [NEW] | `src/i18n/index.ts` |
| [NEW] | `src/i18n/locales/zh.json` |
| [NEW] | `src/i18n/locales/en.json` |
| [MODIFY] | [src/main.ts](file:///c:/Users/12192/TierMaker/src/main.ts) (注册 i18n) |
| [MODIFY] | [src/App.vue](file:///c:/Users/12192/TierMaker/src/App.vue) |
| [MODIFY] | [src/components/ConfigModal.vue](file:///c:/Users/12192/TierMaker/src/components/ConfigModal.vue) |
| [MODIFY] | [src/components/SearchModal.vue](file:///c:/Users/12192/TierMaker/src/components/SearchModal.vue) |
| [MODIFY] | [src/components/EditItemModal.vue](file:///c:/Users/12192/TierMaker/src/components/EditItemModal.vue) |
| [MODIFY] | [src/components/ExportModal.vue](file:///c:/Users/12192/TierMaker/src/components/ExportModal.vue) |
| [MODIFY] | [src/components/ImportModal.vue](file:///c:/Users/12192/TierMaker/src/components/ImportModal.vue) |
| [MODIFY] | [src/utils/storage.ts](file:///c:/Users/12192/TierMaker/src/utils/storage.ts) (等级处理逻辑) |
